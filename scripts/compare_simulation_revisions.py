...
def evaluate_current_worktree(
    ...
    *,
    ...
    params = _configure_params(
        init_params(film_profile=film_profile, print_profile=print_profile)
    )
    ...
    results["neg_scan_midgray"] = np.mean(
        simulate(_patch((0.184, 0.184, 0.184)), params), axis=(0, 1)
    ).tolist()

    params = _configure_params(
        init_params(film_profile=positive_film_profile, print_profile=print_profile)
    )
    params.workflow.route = "input > film > scan"
    for name, rgb in {
        "pos_scan_midgray": (0.184, 0.184, 0.184),
    ...
    for previous, current in zip(revisions, revisions[1:]):
        ...
        for key in keys:
            distance = _vector_distance(
                results_by_revision[previous][key], results_by_revision[current][key]
            )
...
def _plot_patch_grid(
    results_by_revision: dict[str, dict[str, list[float]]],
    ...
    *,
    ...
    revisions = list(results_by_revision)
    patch_names = [
        name for name in PLOT_PATCH_ORDER if name in results_by_revision[revisions[0]]
    ]
    if not patch_names:
        return
    ...
    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(2.6 * ncols, 1.8 * nrows)
    )
    ...
    for row_index, patch_name in enumerate(patch_names):
        ...
        input_rgb_display = np.clip(
            _encode_srgb_for_display(input_rgb_linear), 0.0, 1.0
        )
        ...
        for col_index, revision in enumerate(revisions, start=1):
            rgb_linear = np.asarray(
                results_by_revision[revision][patch_name], dtype=np.float64
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare deterministic simulation outputs across git revisions."
    )
    parser.add_argument(
        "revisions", nargs="*", help="Git revisions to compare in order."
    )
    ...
    parser.add_argument(
        "--plot-file",
        type=Path,
        help="Optional output path for a visual patch comparison plot.",
    )
    parser.add_argument(
        "--show-plot",
        action="store_true",
        help="Display the patch comparison plot interactively.",
    )
...
def main() -> None:
    ...
    if not args.revisions:
        raise SystemExit(
            "Provide at least one revision or use --evaluate-current-worktree."
        )
