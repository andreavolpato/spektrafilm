...
class TestLoadProfile:
    ...
    def test_profile_data_shapes_are_consistent(self, stock):
        ...
        assert (
            profile.data.density_curves.shape[0] == profile.data.log_exposure.shape[0]
        )
        ...
        assert (
            profile.data.channel_density.shape[0] == profile.data.wavelengths.shape[0]
        )
        ...
        assert (
            profile.data.midscale_neutral_density.shape[0]
            == profile.data.wavelengths.shape[0]
        )

    def test_profile_namespace_round_trip_preserves_core_fields(
        self, portra_400_profile
    ):
        ...
        assert (
            np.array(profile_rt.data.log_exposure).shape
            == portra_400_profile.data.log_exposure.shape
        )
        assert (
            np.array(profile_rt.data.density_curves).shape
            == portra_400_profile.data.density_curves.shape
        )
        assert (
    ...
    def test_profile_json_round_trip_preserves_hanatos2025_adaptation(
        self, portra_400_profile
    ):
        payload = json.dumps(
            _json_safe(profile_to_dict(portra_400_profile)), allow_nan=False
        )
    ...
    def test_profile_update_helpers_replace_nested_dataclasses(
        self, portra_400_profile
    ):
        ...
        np.testing.assert_allclose(
            portra_400_profile.data.channel_density, updated_density
        )
...
class TestFlexibleProfileData:
    ...
    def test_omitted_optional_fields_default_to_none(self):
        profile = profile_from_dict(
            {"info": {"channel_model": "bw"}, "data": {"wavelengths": [380.0, 385.0]}}
        )
