...
def test_load_state_from_file_syncs_canvas_background(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "getOpenFileName",
        staticmethod(lambda *args, **kwargs: ("state.json", "JSON (*.json)")),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda path: PROJECT_DEFAULT_GUI_STATE,
    )
    monkeypatch.setattr(
        controller_module,
        "apply_gui_state",
    ...
    monkeypatch.setattr(
        controller,
        "_sync_canvas_background",
        lambda: captured.setdefault("canvas_synced", True),
    )
...
def test_restore_factory_default_syncs_canvas_background(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        controller_module, "clear_saved_default_gui_state", lambda: None
    )
    monkeypatch.setattr(
        controller_module,
        "apply_gui_state",
...
def test_save_current_as_default_persists_collected_state(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda state: captured.setdefault("state", state),
    )
    monkeypatch.setattr(
        controller_module,
        "set_status",
...
def test_save_current_state_to_file_persists_json(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda state, path: captured.setdefault("saved", (state, path)),
    )
    monkeypatch.setattr(
        controller_module,
        "set_status",
...
def test_load_state_from_file_applies_loaded_state(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        controller_module, "load_gui_state_from_path", lambda path: gui_state
    )
    monkeypatch.setattr(
        controller_module,
        "apply_gui_state",
...
def test_restore_factory_default_clears_saved_default_and_applies_factory_state(
    monkeypatch,
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    captured: dict[str, object] = {}
