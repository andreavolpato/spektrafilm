...
def _configure_save_output(
    ...
    monkeypatch.setattr(
        controller_module,
        "set_status",
        lambda viewer, message: captured.setdefault("status", message),
    )
    monkeypatch.setattr(controller_module, "load_dialog_dir", lambda key: "")
    monkeypatch.setattr(
        controller_module, "save_dialog_dir", lambda key, directory: None
    )
    monkeypatch.setattr(
    ...
    if converted_delta is None:
        ...
        def fail_rgb_to_rgb(*args, **kwargs):
            raise AssertionError(
                "RGB_to_RGB should not be called when color spaces and encoding flags match"
            )
...
def _assert_fallback_preview(
    preview: np.ndarray, status: str, *, expected_status: str
) -> None:
...
def _make_display_transform_controller(
    *, with_toggle: bool
) -> tuple[GuiController, StubToggle | None]:
    if not with_toggle:
...
def test_set_output_interpolation_mode_updates_visible_output_layer(
    monkeypatch,
) -> None:
...
def test_save_output_layer_respects_recorded_render_metadata(
    ...
    if converted_delta is None:
        ...
        np.testing.assert_allclose(
            rgb_to_rgb_call["image_data"], captured["float_image"]
        )
    ...
    np.testing.assert_allclose(
        saved_image, captured["float_image"] + expected_saved_delta
    )
...
def test_prepare_output_display_image_uses_imagecms_transform(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        controller_module.ImageCms, "get_display_profile", lambda: object()
    )
    monkeypatch.setattr(
        controller_module.ImageCms,
        "getProfileName",
        lambda profile: "Studio Display ICC\x00",
    )
    monkeypatch.setattr(
        controller_module.colour,
        "RGB_to_RGB",
        lambda *args, **kwargs: np.full((1, 1, 3), 0.5, dtype=np.float32),
    )
    monkeypatch.setattr(
        controller_module.ImageCms, "createProfile", lambda name: f"profile:{name}"
    )
    monkeypatch.setattr(
        controller_module.PILImage,
        "fromarray",
        lambda array, mode="RGB": captured.setdefault(
            "source_image", FakePILImage(array.copy())
        ),
    )
    ...
    monkeypatch.setattr(
        controller_module.ImageCms, "profileToProfile", fake_profile_to_profile
    )
...
def test_prepare_output_display_image_reports_missing_display_profile(
    monkeypatch,
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    image_data = np.array([[[0.2, 0.4, 0.6]]], dtype=np.float32)
    ...
    _assert_fallback_preview(
        preview,
        status,
        expected_status="Display transform: no display profile, using raw preview",
    )
...
def test_prepare_output_display_image_reports_transform_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        controller_module.PILImage, "fromarray", lambda array, mode="RGB": object()
    )
    ...
    monkeypatch.setattr(
        controller_module.ImageCms, "profileToProfile", raise_transform_error
    )
    ...
    _assert_fallback_preview(
        preview,
        status,
        expected_status="Display transform: transform failed, using raw preview",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            True,
            object(),
            "Adobe RGB Monitor\x00",
            "Display transform: display profile found (Adobe RGB Monitor)",
        ),
    ],
...
def test_report_display_transform_status_messages(
    ...
    monkeypatch.setattr(
        controller_module.ImageCms, "get_display_profile", lambda: display_profile
    )
    if profile_name is not None:
        monkeypatch.setattr(
            controller_module.ImageCms, "getProfileName", lambda profile: profile_name
        )
...
def test_set_gray_18_canvas_enabled_updates_napari_background(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda viewer, *, gray_18_canvas: captured.setdefault(
            "canvas", (viewer, gray_18_canvas)
        ),
    )
...
def test_sync_display_transform_availability_unchecks_when_profile_missing(
    monkeypatch,
) -> None:
    controller, toggle = _make_display_transform_controller(with_toggle=True)
