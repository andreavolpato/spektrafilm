...
def _make_filepicker_section(monkeypatch):
    ...
    monkeypatch.setattr(
        widgets_module,
        "_build_button",
        lambda text, callback, **kwargs: (
            created_buttons.append(
                FakeButton(text, callback, tooltip=kwargs.get("tooltip"))
            )
            or created_buttons[-1]
        ),
    )
    monkeypatch.setattr(
        widgets_module,
        "_build_button_row",
        lambda *widgets, **kwargs: ("button-row", widgets, kwargs),
    )
    monkeypatch.setattr(
        widgets_module,
        "_build_vertical_container",
        lambda *items, **kwargs: ("vertical-container", items, kwargs),
    )
    monkeypatch.setattr(
        widgets_module, "_set_single_collapsible_layout", lambda *args, **kwargs: None
    )
...
def test_file_picker_choose_file_updates_path_and_emits_selected_file(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "getOpenFileName",
        staticmethod(
            lambda *_args, **_kwargs: ("C:/tmp/example.tif", "Images (*.tif)")
        ),
    )
...
def test_scan_for_print_toggle_applies_and_restores_scanner_and_glare_state() -> None:
    toggle_scan_for_print = getattr(
        widgets_module.SimulationSection, "_apply_scan_for_print_mode"
    )
...
def test_numeric_field_specs_define_minimum_and_step() -> None:
    ...
    group_hints = {
        "io": get_type_hints(IOParams),
        "settings": get_type_hints(SettingsParams),
    }
...
def test_param_manifests_are_complete_and_well_formed() -> None:
    ...
    for manifest in ALL_MANIFESTS:
        group_field_names = {
            field_info.name for field_info in fields(manifest.group_cls)
        }
        ...
        for spec in manifest.fields:
            ...
            # declared twice
            assert spec.leaf in group_field_names, (
                f"{spec.path}: not a field of {manifest.group_cls.__name__}"
            )
...
def test_collapsible_section_shows_icon_for_mapped_main_tab_title(monkeypatch) -> None:
    ...
    section = primitives_module.CollapsibleSection(
        "Import RGB", QtWidgets.QWidget(), expanded=True
    )
...
def test_simulation_section_profile_use_badges_follow_selected_profiles() -> None:
    ...
    assert section.print_paper.currentText() == list(state_module.PrintStocks)[0].value
    assert (
        widget_editors_module.ProfileEnumEditor.display_text_for_value(
            "kodak_portra_400"
        )
        == "still / neg / color / kodak_portra_400"
    )
    assert (
        widget_editors_module.ProfileEnumEditor.display_text_for_value(
            "kodak_vision3_50d"
        )
        == "cine / neg / color / kodak_vision3_50d"
    )
    assert (
        widget_editors_module.ProfileEnumEditor.display_text_for_value("kodak_2393")
        == "cine / neg / color / kodak_2393"
    )
    assert (
        widget_editors_module.ProfileEnumEditor.display_text_for_value("kodak_trix")
        == "cine / pos / bw / kodak_trix"
    )
