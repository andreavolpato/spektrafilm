...
from spektrafilm_gui.controller import PROFILE_SYNC_SECTION_NAMES, GuiController
...
def _run_simulation_case(
    ...
    *,
    ...
    input_image = (
        np.full((6, 4, 3), 0.25, dtype=np.float32)
    ...
    monkeypatch.setattr(
        controller,
        "_sync_white_border",
        lambda *, white_padding: captured.setdefault("white_padding", white_padding),
    )
    monkeypatch.setattr(
        controller_module, "collect_gui_state", lambda *, widgets: gui_state
    )
    monkeypatch.setattr(
        controller_module, "build_params_from_state", lambda state: object()
    )
    ...
    monkeypatch.setattr(
        controller, "_process_image_with_runtime", fake_process_image_with_runtime
    )
    monkeypatch.setattr(
        controller, "_prepare_output_display_image", fake_prepare_output_display_image
    )
    monkeypatch.setattr(
        controller, "_set_or_add_output_layer", fake_set_or_add_output_layer
    )
...
def test_load_input_image_builds_preview_stack_and_homes_view(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda state: (_ for _ in ()).throw(
            AssertionError("should not build params for image load preview cache")
        ),
    )
    monkeypatch.setattr(
        controller, "_resize_for_preview", lambda image, *, max_size: preview_image
    )
    monkeypatch.setattr(
        controller,
        "_prepare_input_color_preview_image",
        lambda *args, **kwargs: preview_display_image,
    )
    monkeypatch.setattr(
        controller_module,
        "reset_viewer_camera",
        lambda viewer: captured.setdefault("reset_view", True),
    )
...
def test_show_startup_placeholder_builds_portrait_preview_stack_and_homes_view(
    monkeypatch,
) -> None:
    viewer = FakeViewer()
    controller = GuiController(viewer=viewer, widgets=object())
...
def test_load_input_image_requests_auto_preview_once_when_enabled(monkeypatch) -> None:
    controller = GuiController(
        ...
        widgets=SimpleNamespace(
            simulation=SimpleNamespace(auto_preview_value=lambda: True)
        ),
    )
    raw_image = np.full((4, 2, 3), 0.25, dtype=np.float32)
    preview_image = np.full((2, 1, 3), 0.5, dtype=np.float32)
    ...
    monkeypatch.setattr(controller_module, "load_image_oiio", lambda path: raw_image)
    monkeypatch.setattr(
        controller, "_set_or_add_input_stack", fake_set_or_add_input_stack
    )
    monkeypatch.setattr(
        controller,
        "request_auto_preview",
        lambda: (
            captured.setdefault("preview_requests", 0)
            or captured.__setitem__(
                "preview_requests", captured.get("preview_requests", 0) + 1
            )
        ),
    )
...
def test_rotate_input_image_clockwise_rebuilds_preview_hides_output_and_requests_auto_preview(
    monkeypatch,
) -> None:
    controller = GuiController(
        viewer=object(),
    ...
    controller._current_input_image = np.arange(2 * 3 * 3, dtype=np.float32).reshape(
        2, 3, 3
    )
    captured: dict[str, object] = {}
    ...
    np.testing.assert_allclose(
        captured["image"], np.rot90(controller._current_input_image, k=-1)
    )
...
def test_rotate_input_image_counterclockwise_rebuilds_preview_without_auto_preview_when_disabled(
    monkeypatch,
) -> None:
    controller = GuiController(
        viewer=object(),
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            AssertionError("auto preview should stay disabled")
        ),
    )
...
def test_rotate_input_image_noops_without_loaded_input(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("should not update preview without input image")
        ),
    )
    monkeypatch.setattr(
        controller,
        "request_auto_preview",
        lambda: (_ for _ in ()).throw(
            AssertionError("should not request auto preview without input image")
        ),
    )
...
def test_load_raw_image_uses_pipeline_input_settings_and_builds_preview_stack(
    monkeypatch,
) -> None:
    viewer = FakeViewer(
    ...
    monkeypatch.setattr(
        controller_module, "load_and_process_raw_file", fake_load_and_process_raw_file
    )
    monkeypatch.setattr(
        controller_module, "collect_gui_state", lambda *, widgets: gui_state
    )
    monkeypatch.setattr(
        ...
        "build_params_from_state",
        lambda state: (_ for _ in ()).throw(
            AssertionError("should not build params for raw image preview cache")
        ),
    )
    monkeypatch.setattr(
        controller, "_resize_for_preview", lambda image, *, max_size: preview_image
    ...
    monkeypatch.setattr(
        controller,
        "_prepare_input_color_preview_image",
        lambda *args, **kwargs: np.full((2, 1, 3), 0.8, dtype=np.float32),
    )
    monkeypatch.setattr(
        controller_module,
        "reset_viewer_camera",
    ...
    monkeypatch.setattr(
        controller_module,
        "set_status",
        lambda viewer, message, timeout_ms=5000: captured.setdefault(
            "status", (message, timeout_ms)
        ),
    )
...
def test_load_raw_image_requests_auto_preview_once_when_enabled(monkeypatch) -> None:
    ...
    monkeypatch.setattr(controller._thread_pool, "start", lambda task: task.run())
    monkeypatch.setattr(
        controller_module, "load_and_process_raw_file", lambda path, **kwargs: raw_image
    )
    monkeypatch.setattr(
        controller_module,
        "collect_gui_state",
        lambda *, widgets: make_test_controller_gui_state(),
    )
    monkeypatch.setattr(
        controller, "_set_or_add_input_stack", fake_set_or_add_input_stack
    )
...
def test_load_raw_image_reports_invalid_custom_white_balance_without_mutating_layers(
    monkeypatch,
) -> None:
    viewer = FakeViewer(
        [
    ...
    monkeypatch.setattr(
        controller_module, "dialog_parent", lambda viewer: "dialog-parent"
    )
    ...
    assert captured_dialog == {
        ...
        "message": (
            ...
            "ValueError: RAW file does not expose a usable camera XYZ matrix for custom white balance."
        ),
    }
    assert len(viewer.layers) == 1


def test_load_raw_image_reports_when_lens_correction_is_not_applied(
    monkeypatch,
) -> None:
    viewer = FakeViewer(
        [FakeLayer(np.zeros((2, 2, 3), dtype=np.float32), name="older")]
    ...
    monkeypatch.setattr(
        controller, "_resize_for_preview", lambda image, *, max_size: image
    )
    monkeypatch.setattr(
        controller, "_prepare_input_color_preview_image", lambda image, **kwargs: image
    )
...
def test_apply_profile_defaults_routes_through_selection_digest(monkeypatch) -> None:
    controller = GuiController(
        viewer=object(),
        widgets=SimpleNamespace(
            film_chemistry=SimpleNamespace(
                set_development_time_choices=lambda times: None
            ),
    ...
    monkeypatch.setattr(
        controller_module, "build_params_from_state", fake_build_params_from_state
    )
    monkeypatch.setattr(
        controller_module, "digest_after_selection", fake_digest_after_selection
    )
    monkeypatch.setattr(
        controller_module, "gui_state_from_params", fake_gui_state_from_params
    )
    monkeypatch.setattr(
        controller,
        "_apply_profile_sync_state",
        lambda state: captured.setdefault("applied_state", state),
    )
    ...
    assert captured["synced_args"] == (
        digested_params,
        gui_state.selection.film_stock,
        gui_state.selection.print_paper,
    )
        ...
        controller = GuiController(
            ...
            widgets=SimpleNamespace(
                couplers=SimpleNamespace(
                    set_state=lambda value: captured.setdefault("couplers", value)
                ),
                simulation=SimpleNamespace(
                    set_state=lambda value: captured.setdefault("simulation", value)
                ),
            ),
        )
...
def test_run_simulation_uses_cached_preview_input(monkeypatch) -> None:
    ...
    assert (
        captured["white_padding"]
        == make_test_controller_gui_state().gui_only.display.white_padding
    )
...
def test_run_simulation_passes_display_transform_settings(monkeypatch) -> None:
    ...
    np.testing.assert_allclose(
        captured["display_args"]["image_data"],
        np.full((4, 4, 3), 0.5, dtype=np.float32),
    )
    assert (
        captured["display_args"]["output_color_space"]
        == gui_state.simulation.io.output_color_space
    )
    assert captured["display_args"]["use_display_transform"] is True
    assert captured["display_args"]["padding_pixels"] == 0.0
    np.testing.assert_array_equal(
        captured["output_layer"]["image"], np.full((6, 6, 3), 99, dtype=np.uint8)
    )
    np.testing.assert_allclose(
        captured["output_layer"]["float_image"],
        np.full((4, 4, 3), 0.5, dtype=np.float32),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "run_scan",
            {
                "source_layer_name": INPUT_LAYER_NAME,
                "mode_label": "Scan",
                "channel": "scan",
            },
        ),
...
def test_run_preview_and_scan_start_async_simulation(
    monkeypatch, method_name: str, expected_call: dict[str, object]
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(
        controller,
        "_start_simulation",
        lambda *, source_layer_name, mode_label, report_status=True, channel="simulation": (
            captured.setdefault(
                "call",
                {
...
def _stub_simulation_execution(
    controller, monkeypatch, captured: dict[str, object]
) -> None:
    """Run the simulation channel synchronously and capture the request the
    controller submits, standing in for the off-thread runtime call."""
    monkeypatch.setattr(controller._thread_pool, "start", lambda task: task.run())
    monkeypatch.setattr(
        controller, "_set_or_add_output_layer", lambda *args, **kwargs: None
    )

    def fake_execute(request):
        captured["request"] = request
        return controller_module.SimulationResult(
            mode_label=request.mode_label,
            display_image=np.zeros((1, 1, 3), dtype=np.uint8),
            float_image=np.zeros((1, 1, 3), dtype=np.float32),
            output_color_space=request.output_color_space,
            use_display_transform=request.use_display_transform,
            status_message="ok",
        )

    monkeypatch.setattr(controller, "_execute_simulation_request", fake_execute)


def test_start_simulation_reports_persistent_computing_status(monkeypatch) -> None:
    simulation_section = SimpleNamespace(
        preview_button=None, scan_button=None, save_button=None
    )
    ...
    _stub_simulation_execution(controller, monkeypatch, captured)

    controller._start_simulation(
        source_layer_name=INPUT_PREVIEW_LAYER_NAME, mode_label="Preview"
    )

    assert captured["status"] == ("Computing preview...", 0)
    assert captured["white_padding"] == gui_state.gui_only.display.white_padding
    np.testing.assert_allclose(captured["request"].image, np.double(preview_image))


def test_start_simulation_skips_computing_status_for_silent_preview(
    monkeypatch,
) -> None:
    simulation_section = SimpleNamespace(
        preview_button=None, scan_button=None, save_button=None
...
def test_request_auto_preview_schedules_once_and_runs_preview(monkeypatch) -> None:
    ...
    controller = GuiController(
        viewer=object(), widgets=SimpleNamespace(simulation=simulation_section)
    )
    ...
    monkeypatch.setattr(
        controller_module.QTimer, "singleShot", staticmethod(fake_single_shot)
    )
    monkeypatch.setattr(
        controller,
        "_run_preview",
        lambda *, report_status: captured.setdefault("preview_runs", []).append(
            report_status
        ),
    )
...
def test_start_warmup_runs_on_working_thread_and_grays_out_buttons(monkeypatch) -> None:
    ...
    buttons = SimpleNamespace(
        preview_button=FakeButton(), scan_button=FakeButton(), save_button=FakeButton()
    )
    controller = GuiController(
        viewer=object(), widgets=SimpleNamespace(simulation=buttons)
    )
    statuses: list[tuple[str, int]] = []
    monkeypatch.setattr(
        controller_module,
    ...
    def fake_warmup() -> None:
        ...
        assert all(
            not getattr(buttons, name).enabled
            for name in ("preview_button", "scan_button", "save_button")
        )
...
def test_run_scheduled_auto_preview_submits_silent_preview(monkeypatch) -> None:
    ...
    # in-flight simulation (see test_background for the coalescing contract).
    simulation_section = SimpleNamespace(auto_preview_value=lambda: True)
    controller = GuiController(
        viewer=object(), widgets=SimpleNamespace(simulation=simulation_section)
    )
    controller._current_preview_image = np.full((2, 2, 3), 0.25, dtype=np.float32)
    controller._auto_preview_scheduled = True
    runs: list[bool] = []

    monkeypatch.setattr(
        controller, "_run_preview", lambda *, report_status: runs.append(report_status)
    )

    controller._run_scheduled_auto_preview()

    assert runs == [False]
    assert controller._auto_preview_scheduled is False


def test_refresh_preview_cache_recomputes_cached_preview_without_hiding_visible_output(
    monkeypatch,
) -> None:
    viewer = FakeViewer()
    controller = GuiController(viewer=viewer, widgets=object())
    ...
    controller._layers.set_or_add_input_preview_layer(
        old_preview, white_padding=gui_state.gui_only.display.white_padding
    )
    ...
    monkeypatch.setattr(
        ...
        "build_params_from_state",
        lambda state: (_ for _ in ()).throw(
            AssertionError("should not build params for preview refresh cache")
        ),
    )
    monkeypatch.setattr(
        controller, "_resize_for_preview", lambda image, *, max_size: new_preview
    )
    monkeypatch.setattr(
        controller,
        "_prepare_input_color_preview_image",
        lambda *args, **kwargs: preview_display,
    )
    monkeypatch.setattr(
        controller_module,
        "reset_viewer_camera",
        lambda viewer: (_ for _ in ()).throw(AssertionError("should not home viewer")),
    )
...
def test_update_preview_cache_uses_input_image_shape_for_watermark_layer(
    monkeypatch,
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    gui_state = make_test_controller_gui_state()
    ...
    monkeypatch.setattr(
        ...
        fake_set_or_add_input_preview_layer,
    )
...
def test_start_simulation_sets_preview_mode_before_runtime_digest(
    ...
    monkeypatch.setattr(
        controller_module, "build_params_from_state", lambda state: params
    )
    monkeypatch.setattr(controller_module, "set_status", lambda *args, **kwargs: None)
    _stub_simulation_execution(controller, monkeypatch, captured)
...
def test_execute_simulation_request_routes_through_runtime_simulator_path(
    monkeypatch,
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    ...
    monkeypatch.setattr(
        ...
        lambda image, **kwargs: (
            np.uint8(np.clip(image, 0.0, 1.0) * 255),
...
def test_process_image_with_runtime_reuses_cached_simulator(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        controller_module, "runtime_simulator", lambda params: FakeSimulator(params)
    )
...
def test_process_image_with_runtime_reapplies_stock_specific_digest_after_profile_change(
    monkeypatch,
) -> None:
    controller = GuiController(viewer=object(), widgets=object())
    params = object()
...
def test_on_simulation_finished_reports_completed_status(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda image, **kwargs: captured.setdefault("output", (image, kwargs)),
    )
    monkeypatch.setattr(
        controller_module,
        "set_status",
    ...
    assert captured["status"] == (
        "Preview completed. Display transform: disabled",
        5000,
    )


def test_on_simulation_finished_skips_completed_status_for_silent_preview(
    monkeypatch,
) -> None:
    controller = GuiController(
        viewer=object(),
