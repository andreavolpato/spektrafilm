...
def test_execute_simulation_request_uses_runtime_runner_without_padding() -> None:
    ...
    result = runtime_module.execute_simulation_request(
        request,
        run_simulation_fn=lambda image, params: np.full(
            (4, 4, 3), 0.5, dtype=np.float32
        ),
        prepare_output_display_image_fn=lambda image, **kwargs: _capture_preview_result(
            captured, image, **kwargs
        ),
    )

    np.testing.assert_allclose(
        captured["display_args"]["image"], np.full((4, 4, 3), 0.5, dtype=np.float32)
    )
    assert result.mode_label == "Preview"
    np.testing.assert_allclose(
        result.float_image, np.full((4, 4, 3), 0.5, dtype=np.float32)
    )
...
def test_prepare_input_color_preview_image_converts_to_srgb_float_preview() -> None:
    ...
    assert (
        captured["call"]["output_color_space"]
        == runtime_module.DISPLAY_PREVIEW_COLOR_SPACE
    )
