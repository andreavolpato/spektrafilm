...
def _make_state() -> GuiState:
    ...
    state.scanner = replace(
        state.scanner, white_correction=True, white_level=0.9, lens_blur=0.3
    )
    state.input_gamut_compress = replace(
        state.input_gamut_compress, active=False, boundary="locus", hull_detail=6.0
    )
    state.output_gamut_compress = replace(
        state.output_gamut_compress, algorithm="aces_rgc"
    )
...
def _make_widgets(state: GuiState) -> WidgetBundle:
    return WidgetBundle(
        ...
        convert=StubSection(clone_state_section(state.convert)),
        glare=StubSection(clone_state_section(state.glare)),
        scanner=StubSection(clone_state_section(state.scanner)),
        input_gamut_compress=StubSection(
            clone_state_section(state.input_gamut_compress)
        ),
        output_gamut_compress=StubSection(
            clone_state_section(state.output_gamut_compress)
        ),
...
def test_apply_gui_state_updates_all_sections() -> None:
    ...
    for section_name in GUI_STATE_SECTION_NAMES:
        assert widgets.__getattribute__(section_name).get_state() == _section_state(
            source_state, section_name
        )
    assert (
        widgets.simulation.auto_preview_value()
        is source_state.simulation.workflow.auto_preview
    )
