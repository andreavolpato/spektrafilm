...
def density_transfer_curves(
    ...
    *,
    ...
    fig, axes_2d = plt.subplots(
        2, 2, figsize=(13, 10), facecolor=BG, layout="constrained"
    )
    ...
    for ax, (label, sweep_data, axis_color) in zip(axes[:3], setups):
        ...
        for pin, samples, alpha in sweep_data:
            ...
            ax.plot(
                sweep_x, density[:, 0], color=RED, lw=2.0, alpha=alpha, zorder=2 + alpha
            )
            ax.plot(
                sweep_x,
                density[:, 1],
    ...
    ax_n.set_title(
        "neutral (R=G=B) sweep", color=HI, fontsize=PANEL_TITLE_FS, pad=PANEL_TITLE_PAD
    )
    _setup_2d(ax_n)
    ax_n.legend(
        facecolor="#1a1a1a",
...
def oklab_ab_slices(grid_output: np.ndarray, out_cs: str) -> Figure:
    ...
    fig.suptitle("OkLab a*–b* slices of the LUT output", color=HI, fontsize=SUPTITLE_FS)
...
def hue_twist_oklab(
    ...
    ax.set_title(
        "Hue-twist diagram (OkLab)", color=HI, fontsize=SUPTITLE_FS, pad=SUPTITLE_PAD
    )
    _setup_2d(ax)
    ax.legend(
        facecolor="#1a1a1a", labelcolor=FG, framealpha=0.9, loc="upper left", fontsize=9
    )
    ax.set_aspect("equal")
    return fig
...
def oklab_displacement(
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(16, 5.5), facecolor=BG, layout="constrained"
    )
    for i, (ax, title) in enumerate(zip(axes, band_titles)):
        ...
        _setup_2d(ax)
    fig.suptitle(
        "OkLab displacement: where the LUT sends each input chromaticity",
        color=HI,
...
def chromaticity_1931(
    ...
    *,
    ...
    ax.set_title(
        "CIE 1931 chromaticity — LUT output footprint",
        color=HI,
        fontsize=SUPTITLE_FS,
        pad=SUPTITLE_PAD,
    )
...
def planckian_path(
    ...
    ax.set_ylabel("y", color=FG)
    ax.set_title(
        "Planckian / daylight sweep — output chromaticity per CCT",
        color=HI,
        fontsize=SUPTITLE_FS,
        pad=SUPTITLE_PAD,
    )
...
def dynamic_range_curve(
    ...
    *,
    ...
    if enc_idx.size >= 2:
        ...
        toe_n = int(
            round(
                stats.get("toe_collapsed_stops", 0.0) / max((stops[1] - stops[0]), 1e-9)
            )
        )
        shoulder_n = int(
            round(
                stats.get("shoulder_collapsed_stops", 0.0)
    ...
    if np.any(encoded_clip_mask):
        ax.plot(
            stops[encoded_clip_mask],
            density[encoded_clip_mask],
            color=DIM,
            lw=1.6,
            marker="x",
            ms=4,
    ...
    ax.set_xlabel(
        "scene-linear stops above camera middle gray  (log₂(linear / 0.18))", color=FG
    )
...
def spectral_locus_envelope(
    ...
    ax.set_facecolor(BG)
    ax.plot(
        locus_xy[:, 0], locus_xy[:, 1], color="#aaaaaa", lw=1.4, label="spectral locus"
    )
    ax.plot(
        [locus_xy[-1, 0], locus_xy[0, 0]],
        [locus_xy[-1, 1], locus_xy[0, 1]],
    ...
    ax.set_ylabel("y", color=FG)
    ax.set_title(
        "Spectral-locus envelope — cube edges through the LUT",
        color=HI,
        fontsize=SUPTITLE_FS,
        pad=SUPTITLE_PAD,
    )
