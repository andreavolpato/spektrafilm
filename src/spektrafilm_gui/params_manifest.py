...
INPUT_GAMUT_COMPRESS_MANIFEST = GroupManifest(
    ...
    fields=(
        ...
        ParamSpec(
            ...
            tooltip="xy: radial compression in CIE 1931 chromaticity toward the spectral locus / inscribed hull (ACES RGC family, preserves dominant wavelength). The only input algorithm.",
        ),
        ParamSpec(
            f"{_IGC}.knee",
            min=0.0,
            step=0.05,
            decimals=3,
            tooltip="Reinhard knee (threshold, limit, power). Default (0.815, 1.0, 1.2) — ACES RGC pair: the 0.815 threshold leaves in-gamut colours untouched, the 1.2 power gives a slow tail so deep-imaginary colours keep their gradation, and limit 1.0 asymptotes on the boundary.",
        ),
        ParamSpec(
            f"{_IGC}.hull_detail",
            tier="advanced",
            ...
            step=0.5,
            decimals=1,
            tooltip="Detail of the inscribed-hull compression boundary = number of FFT modes kept. HIGHER keeps more locus detail (more cusp chroma) but re-admits polygon kinks; LOWER is smoother, rounding the spectral tips inward toward a circle. Note: this is modes-kept, not a blur width — larger means LESS smoothing.",
        ),
    ),
)
