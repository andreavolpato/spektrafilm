...
def plot(self):
    ...
    if self.type == "negative" and getattr(self, "support", "film") == "film":
        ...
        axs[2].plot(
            self.wavelengths, density_matrix[:, 4], color="lightgray", linewidth=1
        )
...
def plot_midgray_density_test(self, exposure_bias=1):
    ...
    if np.logical_or(
        self.type == "positive", getattr(self, "support", "film") == "paper"
    ):
        ax.plot(wavelengths, np.sum(density_matrix[:, :3], axis=1), label="Midgray")
        ax.plot(
            wavelengths, density_midgray[0, 0], color="k", label="Midgray Simulated"
        )
        ax.legend()
...
def grain_test(self, multilayer_grain=True, samples=256):
    ...
    film_format_equivalent = (
        pixel_size_equivalent * np.max(gradient_image.shape[0:2]) / 1000
    )  # mm
...
def test_exposure_bias(self, illuminant):
    ...
    ax.plot(
        np.log10(gray_scale),
        np.mean(gray_scale_image, axis=2)[:, 0],
        color="gray",
        linestyle="--",
    )
...
def plot_chromaticity(self, ax=None, color="k"):
    raise NotImplementedError(
        "plot_chromaticity is still a draft helper and needs a dedicated implementation."
    )
