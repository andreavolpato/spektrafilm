...
def _fill_monotone_slopes_1d(values, slopes):
    ...
    if right * deltas[size - 2] <= 0.0:
        right = 0.0
    elif deltas[size - 2] * deltas[size - 3] < 0.0 and abs(right) > abs(
        3.0 * deltas[size - 2]
    ):
...
@njit(cache=True)
def _pchip_interp_lut_at_3d_prepared(
    lut, slope_x, slope_y, slope_z, cell_min, cell_max, r, g, b
):
    size = lut.shape[0]
    ...
    for c in range(lut.shape[3]):
        ...
        sy11 = _linear_mix(
            slope_y[i, j + 1, k + 1, c], slope_y[i + 1, j + 1, k + 1, c], tr
        )
        ...
        sz1 = _bilinear_mix(
            ...
            slope_z[i + 1, j, k + 1, c],
            slope_z[i, j + 1, k + 1, c],
            slope_z[i + 1, j + 1, k + 1, c],
            tr,
            tg,
        )

        interpolated = _hermite_value(vz0, vz1, sz0, sz1, tb)
        out[c] = _clamp_to_range(
            interpolated, cell_min[i, j, k, c], cell_max[i, j, k, c]
        )
    return out


@njit(parallel=True, cache=True)
def _apply_lut_pchip_3d_prepared(
    lut, slope_x, slope_y, slope_z, cell_min, cell_max, image
):
    height, width, _ = image.shape
    n_out = lut.shape[3]
    output = np.empty((height, width, n_out), dtype=lut.dtype)
    ...
    for i in prange(height):
        for j in range(width):
            ...
            g_in = image[i, j, 1] * scale
            b_in = image[i, j, 2] * scale
            out_val = _pchip_interp_lut_at_3d_prepared(
                lut, slope_x, slope_y, slope_z, cell_min, cell_max, r_in, g_in, b_in
            )
            for c in range(n_out):
                output[i, j, c] = out_val[c]
    return output
...
def apply_lut_pchip_3d(lut, image):
    ...
    return _apply_lut_pchip_3d_prepared(
        lut, slope_x, slope_y, slope_z, cell_min, cell_max, image
    )
    ...
    if lut.ndim == 4:  # 3D LUT case
        ...
        for c in range(3):
            output[:, :, c] = map_coordinates(
                lut[..., c], coords, order=3, mode="reflect"
            )
        return output
...
if __name__ == "__main__":
    ...
    grid_r_3d, grid_g_3d, grid_b_3d = np.meshgrid(
        grid_3d, grid_3d, grid_3d, indexing="ij"
    )
    # Create a 3D LUT that applies a simple non-linear transformation (r^2, g^2, b^2)
    lut_3d = np.stack(
        (grid_r_3d**2, grid_g_3d**2, grid_b_3d**2), axis=-1
    )  # shape: (L, L, L, 3)
    ...
    print(
        f"3D LUT - Average time per iteration (default raw LUT path): {default_time_3d:.6f} seconds"
    )
    ...
    print(
        f"3D LUT - Average time per iteration (Mitchell cubic interpolation): {mitchell_time_3d:.6f} seconds"
    )
    ...
    print(
        f"3D LUT - Average time per iteration (PCHIP cubic Hermite): {pchip_time_3d:.6f} seconds"
    )
    ...
    print(
        f"3D LUT - Average time per iteration (SciPy cubic interpolation): {scipy_time_3d:.6f} seconds"
    )
    ...
    print(
        f"3D LUT - Default-path RMSE error against SciPy: {rmse_default_scipy_3d:.6e}"
    )
    print(
        f"3D LUT - Default-path max absolute error against SciPy: {max_error_default_scipy_3d:.6e}"
    )
    ...
    print(f"3D LUT - Mitchell RMSE error against SciPy: {rmse_mitchell_scipy_3d:.6e}")
    print(
        f"3D LUT - Mitchell max absolute error against SciPy: {max_error_mitchell_scipy_3d:.6e}"
    )
    ...
    print(
        f"3D LUT - Default-path RMSE error against ground truth: {rmse_default_ground_truth_3d:.6e}"
    )
    print(
        f"3D LUT - Default-path max absolute error against ground truth: {max_error_default_ground_truth_3d:.6e}"
    )
    ...
    print(
        f"3D LUT - PCHIP RMSE error against ground truth: {rmse_pchip_ground_truth_3d:.6e}"
    )
    print(
        f"3D LUT - PCHIP max absolute error against ground truth: {max_error_pchip_ground_truth_3d:.6e}"
    )
    ...
    print(
        f"3D LUT - SciPy RMSE error against ground truth: {rmse_scipy_ground_truth_3d:.6e}"
    )
    print(
        f"3D LUT - SciPy max absolute error against ground truth: {max_error_scipy_ground_truth_3d:.6e}"
    )

    diff_norm_ground_truth_3d = np.sqrt(np.sum(diff_default_ground_truth_3d**2, axis=2))
    diff_norm_scipy_ground_truth_3d = np.sqrt(
        np.sum(diff_scipy_ground_truth_3d**2, axis=2)
    )
    fig, axs = plt.subplots(2, 2, figsize=(14, 12))
    input_im = axs[0, 0].imshow(image_3d, interpolation="nearest")
    axs[0, 0].set_title("Input Gradient Image (3D LUT)")
    axs[0, 0].axis("off")
    output_ground_truth_im = axs[0, 1].imshow(
        output_ground_truth_3d, interpolation="nearest"
    )
    axs[0, 1].set_title("Output (Ground Truth, 3D LUT)")
    axs[0, 1].axis("off")
    im_numba = axs[1, 0].imshow(
        diff_norm_ground_truth_3d, cmap="hot", interpolation="nearest"
    )
    axs[1, 0].set_title("Error Map (Default/PCHIP vs Ground Truth, 3D LUT)")
    axs[1, 0].axis("off")
    fig.colorbar(im_numba, ax=axs[1, 0], fraction=0.046, pad=0.04)
    im_scipy = axs[1, 1].imshow(
        diff_norm_scipy_ground_truth_3d, cmap="hot", interpolation="nearest"
    )
    ...
    print(
        f"2D LUT - Average time per iteration (Numba cubic interpolation): {numba_time_2d:.6f} seconds"
    )
    ...
    print(
        f"2D LUT - Average time per iteration (SciPy cubic interpolation): {scipy_time_2d:.6f} seconds"
    )
    ...
    print(f"2D LUT - RMSE error between Numba and SciPy outputs: {rmse_2d:.6e}")
    print(
        f"2D LUT - Max absolute error between Numba and SciPy outputs: {max_error_2d:.6e}"
    )
    ...
    print(f"2D LUT - RMSE error against ground truth: {rmse_ground_truth_2d:.6e}")
    print(
        f"2D LUT - Max absolute error against ground truth: {max_error_ground_truth_2d:.6e}"
    )
    ...
    print(
        f"2D LUT - SciPy RMSE error against ground truth: {rmse_scipy_ground_truth_2d:.6e}"
    )
    print(
        f"2D LUT - SciPy max absolute error against ground truth: {max_error_scipy_ground_truth_2d:.6e}"
    )

    diff_norm_ground_truth_2d = np.sqrt(np.sum(diff_ground_truth_2d**2, axis=2))
    diff_norm_scipy_ground_truth_2d = np.sqrt(
        np.sum(diff_scipy_ground_truth_2d**2, axis=2)
    )
    ...
    output_ground_truth_im = axs[0, 1].imshow(
        output_ground_truth_2d_gray, interpolation="nearest", cmap="gray"
    )
    axs[0, 1].set_title("Output (Ground Truth, 2D LUT, Mean)")
    axs[0, 1].axis("off")
    fig.colorbar(output_ground_truth_im, ax=axs[0, 1], fraction=0.046, pad=0.04)
    im_numba = axs[1, 0].imshow(
        diff_norm_ground_truth_2d, cmap="hot", interpolation="nearest"
    )
    axs[1, 0].set_title("Error Map (Numba vs Ground Truth, 2D LUT)")
    axs[1, 0].axis("off")
    fig.colorbar(im_numba, ax=axs[1, 0], fraction=0.046, pad=0.04)
    im_scipy = axs[1, 1].imshow(
        diff_norm_scipy_ground_truth_2d, cmap="hot", interpolation="nearest"
    )
