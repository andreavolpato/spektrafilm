...
def measure_slopes_at_exposure(
    ...
    for i in range(3):
        ...
        density_1 = CubicSpline(log_exposure[sel], density_curves[sel, i])(
            log_exposure_1
        )
        density_0 = CubicSpline(log_exposure[sel], density_curves[sel, i])(
            log_exposure_0
        )
...
def measure_density_min(log_exposure, density_curves, info_type, control_plot=False):
    ...
    for i in np.arange(n_ch):
        ...
        data[
            data
            > ((np.nanmax(data) - np.nanmin(data)) * fraction_to_fit + np.nanmin(data))
        ] = np.nan
    ...
    if control_plot:
        ...
        for i in range(n_ch):
            ...
            axs[i].axhline(
                density_min[i], color="r", linestyle="--", label="density min"
            )
