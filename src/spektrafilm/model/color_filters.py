...
class DichroicFilters:
    ...
    def apply(self, illuminant, filter_transmittance_values=[1, 1, 1]):
        ...
        dimmed_filters = 1 - (1 - self.filters) * (
            1 - np.array(filter_transmittance_values)
        )
    ...
    def apply_cc(self, illuminant, filter_cc_values=[0, 0, 0]):
        ...
        return self.apply(
            illuminant, filter_transmittance_values=filter_transmittance_values
        )

    def create_custom_filters(
        self, edges=[516, 500, 610, 607], transitions=[12, 8, 8, 8]
    ):
        self.filters = create_combined_dichroic_filter(
            self.wavelengths, transitions=transitions, edges=edges
        )
...
class GenericFilter:
    ...
    def plot(self, ax=None, label=None, **kwargs):
        if ax is None:
            _, ax = plt.subplots()
        ax.plot(
            self.wavelengths, self.transmittance, label=label or self.name, **kwargs
        )
        ax.set_xlabel("Wavelength (nm)")
        ax.set_ylabel("Transmittance")
        ax.set_ylim(0, 1)
...
schott_kg1_heat_filter = GenericFilter(
    name="KG1", filter_type="heat_absorbing", brand="schott"
)
schott_kg3_heat_filter = GenericFilter(
    name="KG3", filter_type="heat_absorbing", brand="schott"
)
schott_kg5_heat_filter = GenericFilter(
    name="KG5", filter_type="heat_absorbing", brand="schott"
)
generic_lens_transmission = GenericFilter(
    name="canon_24_f28_is",
    filter_type="lens_transmission",
    brand="canon",
    data_in_percentage=True,
)
...
COLOR_FILTER_SPECS: dict[str, ColorFilterSpec] = {
    spec.key: spec for spec in COLOR_FILTER_LIBRARY
}
...
CameraColorFilters = Enum(
    "CameraColorFilters",
    {
        ...
        **{spec.key: spec.key for spec in COLOR_FILTER_LIBRARY},
    },
)


def color_enlarger(
    light_source, filter_cc_values=(0, 65, 55), filters=custom_dichroic_filters
):
    # Filter values are in Kodak CC units proportional to density, 100 units means 1.0 density, or 90% reduction in transmittance
    # cc_filter_values are in CMY order
    filter_cc_values = np.array(filter_cc_values)
    filtered_illuminant = filters.apply_cc(
        light_source, filter_cc_values=filter_cc_values
    )
    return filtered_illuminant
