...
def black_body_spectrum(temperature):
    values = colour.colorimetry.blackbody_spectral_radiance(
...
def standard_illuminant(type="D65", return_class=False):
    if type[0:2] == "BB":
        ...
        spectral_intensity = (
            colour.SDS_LIGHT_SOURCES["Incandescent"].copy().align(SPECTRAL_SHAPE)
        )
    elif type == "K75P":
        spectral_intensity = (
            colour.SDS_LIGHT_SOURCES["Kinoton 75P"].copy().align(SPECTRAL_SHAPE)
        )
    elif type == "TH-KG3":
        spectral_intensity = black_body_spectrum(3400)
        spectral_intensity.values = schott_kg3_heat_filter.apply(
            spectral_intensity.values
        )
        ...
        spectral_intensity.values = generic_lens_transmission.apply(
            spectral_intensity.values
        )
    ...
    # normalization
    normalization = np.sum(spectral_intensity.values) / np.size(
