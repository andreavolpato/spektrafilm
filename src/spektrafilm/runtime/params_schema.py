...
class DirCouplersParams:
    ...
    # Langmuir saturating-donor shape knob per channel, normalized by donor
    # d_max: the absolute half-saturation scale is K = langmuir_k_rgb * d_max
    # (computed in-model from the density curves). Amplitude-matched at
    # D_ref = d_max/2, so the operating point is unchanged vs. linear; the
    # inhibitor release rolls off above D_ref instead of growing without bound.
    # langmuir_k_rgb -> inf recovers the linear law.
    langmuir_k_rgb: tuple[float, float, float] = (1.0, 1.0, 1.0)
