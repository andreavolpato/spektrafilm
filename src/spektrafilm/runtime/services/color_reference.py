...
class ColorReferenceService:
    ...
    def __init__(
        ...
        self.cmy_to_log_xyz: Any | None = (
            None  # callable to convert cmy densities to log xyz, to be defined in the scanning stage during pipeline init!
        )
    ...
    def _update_cmy_black_white_references(self, in_print=False):
        if not self._black_correction and not self._white_correction:
            ...
            if self.cmy_to_log_xyz is None:
                raise RuntimeError(
                    "cmy_to_log_xyz must be configured before black/white correction is used."
                )
            ...
            if self._scan_film and self._film.info.type == "positive" and not in_print:
                cmy_black = np.nanmax(self._film.data.density_curves, axis=0)[
                    None, None, :
                ]
                ...
                not self._scan_film and self._print.info.type == "negative" and in_print
            ):
    ...
    def black_white_filming_exposure_correction(self):  # in filming and printing
        if not self._black_correction and not self._white_correction:
            ...
            log_exposure_midgray_corrected = -np.interp(
                -(density_midgray_corrected - density_min_av),
                -density_curve_av,
                log_exposure,
            )
            log_exposure_midgray = -np.interp(
                -(density_midgray - density_min_av), -density_curve_av, log_exposure
            )
            exposure_correction = 10 ** (
                log_exposure_midgray_corrected - log_exposure_midgray
            )
